 ...
 */
export async function updateUserScore(userId: string, xp: number): Promise<void> {
 ...
 */
export async function getUserRank(userId: string): Promise<{ posicao: number | null; xp: number }> {
  try {
    ...
    logger.error({ error, userId }, `Erro ao buscar posição do usuário ${userId}`)
    throw error
  }
}
